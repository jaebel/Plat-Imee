...
internal fun MockMvc.createAnime(objectMapper: ObjectMapper, anime: AnimeCreateDTO): MvcResult =
    this.post("/api/v1/anime") {
        content = objectMapper.writeValueAsString(anime)
        contentType = MediaType.APPLICATION_JSON
    }.andReturn()

internal fun MockMvc.updateAnime(objectMapper: ObjectMapper, anime: AnimeUpdateDTO, animeId: Long): MvcResult =
    this.patch("/api/v1/anime/$animeId") {
        content = objectMapper.writeValueAsString(anime)
        contentType = MediaType.APPLICATION_JSON
    }.andReturn()
