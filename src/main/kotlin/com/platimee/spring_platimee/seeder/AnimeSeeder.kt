...
class AnimeSeeder(
    ...
    private fun seedAnimeFromCsv() {
        ...
        if (!resource.exists()) {
            println("CSV file not found! Skipping seeding.")
            return
        }

        try {
            CSVReader(InputStreamReader(resource.inputStream)).use { reader ->
                // Read header line
            ...
            println("Error while reading anime.csv — skipping seeding. Reason: ${ex.message}")
        }
    }
}
